...
- `teacher`：教师 / 监考人员，具备考试监考与成绩录入等权限。
- `student`：考生，具备报名、参加考试和查看成绩的基础权限。
- 后端通过解析 JWT 中的 `roles` 数组执行权限校验。

## 3. 数据库表结构（PostgreSQL）
详见 `db/schema.sql`，主要表包含：
- `users`：存储登录凭据与基础状态。
- `roles`：系统角色定义。
- `user_roles_link`：多对多关联表。
- `student_profiles`、`teacher_profiles`、`admin_profiles`：角色专属扩展信息。

### 关键约束
- `users.phone`、`users.username` 唯一约束，满足手机号或用户名登录需求。
...
3. Resolver 顶部检查 `roles`，不满足条件时抛出未授权异常。

## 6. 后续扩展方向
- 独立 `permissions` 表与多级资源粒度控制。
- 审计日志与操作留痕。
- 多租户或地域化数据隔离策略。
