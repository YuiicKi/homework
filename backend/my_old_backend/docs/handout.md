...
| `login` | Mutation | `loginIdentifier: String!`<br>`password: String!` | 公共 | 通过手机号或用户名登录，返回 JWT 与用户信息。 |
...
| `adminCreateUser` | Mutation | `input: AdminCreateUserInput!`<br>（可带 `phone`/`username`、`roleName`、`fullName`、`staffId`、部门等） | `admin` | 管理员创建考务/教师/学生账号，并补齐 Profile。 |
| `updateUser` | Mutation | `id: ID!`<br>`input: UpdateUserInput!` | `admin` | 更新手机号、用户名、启用状态。 |
...
4. **权限检查**：各 Resolver 使用 `ensureAuthenticated` / `ensureHasRole(context, 'admin')` 判断是否登录、是否有指定角色；没有权限会抛出 “未登录” 或 “无权访问”。
5. **角色拓展**：管理员账号可调用 `assignRoleToUser`、`removeRoleFromUser` 动态调整权限，JWT 中的角色集合用于前端/后端判断资源可见性。
