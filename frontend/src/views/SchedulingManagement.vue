  ...
  <div class="scheduling-container">
    ...
    <el-tabs v-model="activeTab" type="card" class="demo-tabs">
      ...
      <el-tab-pane label="排考设置" name="schedules">
        <div class="operation-bar">
          <el-button type="primary" @click="showScheduleDialog = true">
            <el-icon style="margin-right: 5px"><Plus /></el-icon> 添加考场绑定
          </el-button>
          <el-button @click="refetchSchedules">刷新列表</el-button>
        </div>
        <el-alert title="请在此处将科目、场次与具体教室绑定。排座算法将仅使用已绑定的教室。" type="info" style="margin-bottom: 15px" :closable="false" />
        
        <el-table :data="schedules" border v-loading="schedulesLoading">
          <el-table-column label="科目" width="200">
            <template #default="{ row }">{{ row.examSubject?.name }}</template>
          </el-table-column>
          <el-table-column label="场次" width="150">
            <template #default="{ row }"><el-tag effect="plain">{{ row.examSession?.name }}</el-tag></template>
          </el-table-column>
          <el-table-column label="使用教室">
            <template #default="{ row }">
          ...
          <el-table-column label="容量" width="100" align="center">
            <template #default="{ row }">{{ row.examRoom?.capacity }}</template>
          </el-table-column>
          <el-table-column label="操作" width="120" align="center">
            <template #default="{ row }">
              <el-button type="danger" size="small" @click="handleDeleteSchedule(row)">解除绑定</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="通知公告" name="notifications">
        <div class="operation-bar">
          <el-button type="primary" @click="showNotifyDialog = true">新建通知</el-button>
          <el-button @click="refetchNotifications">刷新列表</el-button>
        </div>
        <el-table :data="notifications" border v-loading="notifyLoading">
    ...
    <el-dialog v-model="showScheduleDialog" title="绑定考试教室" width="500px">
      <el-form :model="scheduleForm" label-width="100px">
        <el-form-item label="选择场次" required>
          <el-select v-model="scheduleForm.sessionId" placeholder="请选择">
            <el-option v-for="s in sessions" :key="s.id" :label="s.name" :value="s.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="选择科目" required>
          <el-select v-model="scheduleForm.subjectId" placeholder="请选择">
            <el-option v-for="s in subjects" :key="s.id" :label="s.name" :value="s.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="选择教室" required>
          <el-select v-model="scheduleForm.roomId" placeholder="请选择">
            <el-option v-for="r in examRooms" :key="r.id" :label="`${r.name} (容纳${r.capacity}人)`" :value="r.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="scheduleForm.note" placeholder="选填" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="showScheduleDialog = false">取消</el-button>
        <el-button type="primary" @click="handleCreateSchedule" :loading="scheduleSubmitting">确认绑定</el-button>
      </template>
    </el-dialog>
...
import { Document, Tools, Printer, Plus } from '@element-plus/icons-vue'
import dayjs from 'dayjs' 
import AdmitCard from './AdmitCard.vue' 

const activeTab = ref('subjects')
const { client } = useApolloClient()
const formatDate = (isoStr: string) => isoStr ? dayjs(isoStr).format('YYYY-MM-DD HH:mm') : '-'
...
// 2. Windows & Sessions (修正：session { id })
...
const GET_SESSIONS = gql`query { examSessions { id, name } }`
...
// 8. Exam Schedules (新增 - 排考管理)
const GET_SCHEDULES = gql`query { examSchedules { id, examSubject { name }, examSession { name }, examRoom { name, roomNumber, capacity }, status } }`
const CREATE_SCHEDULE = gql`mutation($input: ExamScheduleInput!) { createExamSchedule(input: $input) { id } }`
...
const { result: sessionRes, refetch: refetchSessions } = useQuery(GET_SESSIONS) 
...
const handleSubmitTime = async () => {
  ...
  try {
    ...
    const startTimeStr = startObj.format(fmt)
    const endTimeStr = endObj.format(fmt)
...
const fetchSeatStats = async () => {
  ...
  try {
    const { data } = await client.query({
      query: GET_SEAT_STATS,
      variables: { subjectId: seatQuery.subjectId, sessionId: seatQuery.sessionId },
      fetchPolicy: 'network-only'
    })
    seatStats.value = data.seatAssignmentStats
    fetchAssignments()
...
const fetchAssignments = async () => {
  ...
  try {
    ...
    assignments.value = data.seatAssignments || []
  } finally { assignmentsLoading.value = false }
}

const handleAssignSeats = async () => {
  try {
    await ElMessageBox.confirm('确定要执行自动排座算法吗？这将为所有未分配的考生分配考场。', '排座确认')
    assigningSeats.value = true
    await assignSeatsMutate({ input: { subjectId: seatQuery.subjectId, sessionId: seatQuery.sessionId } })
    ElMessage.success('排座完成！')
    fetchSeatStats() 
  } catch(e:any) { ElMessage.error(e.message) } finally { assigningSeats.value = false }
}

const handleResetSeats = async () => {
  try {
    await ElMessageBox.confirm('危险操作：确定要清空所有座次分配吗？学生将需要重新分配。', '重置确认', { type: 'error' })
    await resetSeatsMutate({ subjectId: seatQuery.subjectId, sessionId: seatQuery.sessionId })
    ElMessage.success('已清空分配')
    fetchSeatStats()
  } catch {}
}

// 7. Schedule Logic (新增 - 排考绑定)
const showScheduleDialog = ref(false)
const scheduleSubmitting = ref(false)
const scheduleForm = reactive({ sessionId: '', subjectId: '', roomId: '', note: '' })

const handleCreateSchedule = async () => {
  if(!scheduleForm.sessionId || !scheduleForm.subjectId || !scheduleForm.roomId) {
...
const handleDeleteSchedule = async (row: any) => {
  try {
    await ElMessageBox.confirm('确认解绑该教室？')
    await deleteScheduleMutate({ id: row.id })
...
const handleSubmitTemplate = async () => {
  try {
    await upsertTemplateMutate({ input: { id: isEditTemplate.value ? templateForm.id : null, name: templateForm.name, logoUrl: templateForm.logoUrl, examNotice: templateForm.examNotice } })
...
const mockExams = [
  { 
    subjectName: '高等数学 (A)', 
    startTime: String(dayjs().add(1, 'day').hour(9).minute(0).valueOf()), 
    endTime: String(dayjs().add(1, 'day').hour(11).minute(0).valueOf()), 
    roomName: '第一教学楼 101', 
    seatNumber: '01' 
  },
  { 
    subjectName: '大学英语 (IV)', 
    startTime: String(dayjs().add(2, 'day').hour(14).minute(30).valueOf()), 
    endTime: String(dayjs().add(2, 'day').hour(16).minute(30).valueOf()), 
    roomName: '综合实验楼 B205', 
    seatNumber: '12' 
  }
]
